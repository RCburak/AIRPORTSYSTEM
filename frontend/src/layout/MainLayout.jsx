...
import { LayoutDashboard, Plane, Activity, Calendar } from 'lucide-react'; // npm install lucide-react
...
export default function MainLayout({ children, aiMessage, selectedFlight, allEvents }) {
  return (
    <div className="flex h-screen w-full bg-airport-950 text-slate-200 overflow-hidden font-sans selection:bg-airport-blue/30">
      {/* Sol Sidebar (Opsiyonel Mini Navigasyon) */}
      <nav className="w-16 border-r border-slate-800 flex flex-col items-center py-6 space-y-8 bg-airport-900/50 backdrop-blur-xl">
      ...
      <div className="flex-1 flex flex-col min-w-0 p-6 relative">
        <header className="h-14 flex justify-between items-center mb-6 px-2">
          <div className="flex flex-col">
            <h1 className="text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 font-black tracking-tight text-xl uppercase">
          ...
          <div className="flex items-center space-x-4">
             ...
             <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-airport-800 to-slate-700 border border-slate-700" />
          </div>
        </header>

        <main className="flex-1 overflow-hidden relative rounded-2xl bg-airport-900/30 border border-slate-800/50 p-1">
          {children}
        </main>
      </div>

      {/* Sağ Panel - AI & Detaylar */}
      <aside className="w-96 border-l border-slate-800 bg-airport-900/40 backdrop-blur-2xl p-6 flex flex-col space-y-6">
        {/* AI Insight Card */}
        <div className="relative overflow-hidden bg-gradient-to-br from-airport-800/80 to-airport-900/80 p-5 rounded-2xl border border-airport-accent/20 shadow-glass">
        ...
        {/* Grafik Alanı */}
        <div className="bg-airport-800/30 rounded-2xl p-2 border border-slate-800/50">
          <OccupancyChart allEvents={allEvents} />
        </div>

        {/* Flight Details Area */}
        <div className="flex-1 bg-airport-950/50 border border-dashed border-slate-700/50 rounded-2xl flex flex-col overflow-hidden shadow-inner">
           ...
           <div className="flex-1 p-6 flex items-center justify-center text-center">
             {selectedFlight ? (
               <div className="space-y-4 animate-in fade-in zoom-in duration-300">
                 <div className="text-3xl font-black text-white tracking-tighter uppercase">{selectedFlight.title}</div>
                 <div className="text-xs px-3 py-1 bg-airport-blue/10 text-airport-blue rounded-full border border-airport-blue/20">
                   Active Operation
                 </div>
               </div>
             ) : (
